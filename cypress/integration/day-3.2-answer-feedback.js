  ...
  context(`Given I submit my answer`, () => {
    ...
    it(`submits my answer typed in the form`, () => {
      ...
      cy.get('main form').within(($form) => {
        cy.get('input#answer-input').type(guess);
    ...
    it(`displays score and feedback the word was incorrect`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within(($main) => {
          cy.get('.total-score').should(
            'have.text',
            `Your total score is: ${incorrectFixture.totalScore}`
          );
          cy.get('.answer-status-subheader').should(
            'have.text',
          ...
          cy.get('.user-guess').should('have.text', `Your answer was ${guess}`);
    ...
    it(`gives feedback the word was correct`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within(($main) => {
          ...
          cy.get('.correct-answer').should(
            'have.text',
            `The correct translation for ${languageHeadFixture.nextWord} was ${correctFixture.answer}`
          );
          cy.get('.user-guess').should('have.text', `Your answer was ${guess}`);
          cy.get('button').should('have.text', `Try another word!`);
        });
